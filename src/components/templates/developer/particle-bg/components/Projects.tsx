            ...
            <div className="space-y-24">
                {data.projects.map((project, index) => (
                    <motion.div
                        ...
                        className={`relative grid md:grid-cols-12 gap-4 items-center ${index % 2 === 1 ? 'text-left md:text-right' : ''}`}
                    >
                        {/* Project Image Area */}
                        <div className={`md:col-span-7 relative h-full min-h-[300px] ${index % 2 === 1 ? 'md:col-start-6' : 'md:col-start-1'}`}>
                            <div className="absolute inset-0 bg-[#64ffda]/20 hover:bg-transparent transition-colors duration-300 rounded z-10 cursor-pointer mix-blend-multiply"></div>
                        ...
                        <div className={`md:col-span-7 relative z-20 ${index % 2 === 1 ? 'md:col-start-1 md:row-start-1' : 'md:col-start-6'}`}>
                            ...
                            <h3 className="text-2xl font-bold text-[#ccd6f6] mb-6 hover:text-[#64ffda] transition-colors cursor-pointer">
                                {project.name}
                            </h3>
                            ...
                            <div className={`flex gap-4 ${index % 2 === 1 ? 'justify-start md:justify-end' : 'justify-start'}`}>
                                {project.github && (
                                    <a href={project.github} target="_blank" rel="noopener noreferrer" className="text-[#ccd6f6] hover:text-[#64ffda] transition-colors">
                                        <Github className="w-5 h-5" />
                                    </a>
                                )}
                                {project.url && (
                                    <a href={project.url} target="_blank" rel="noopener noreferrer" className="text-[#ccd6f6] hover:text-[#64ffda] transition-colors">
